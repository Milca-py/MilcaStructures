...
def calculate_load_vector(modelo: "SystemMilcaModel") -> np.ndarray:
    ...
    # Asignar fuerzas nodales
    for nodo in modelo.node_map.values():
        F[nodo.dof[0]] = nodo.forces.fx
        F[nodo.dof[1]] = nodo.forces.fy
        F[nodo.dof[2]] = nodo.forces.mz

    # Agregar las cargas de los elementos
...
def process_conditions(modelo: "SystemMilcaModel") -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for nodo in modelo.node_map.values():
        restricciones[nodo.dof[0]] = nodo.restraints[0]
        restricciones[nodo.dof[1]] = nodo.restraints[1]
        restricciones[nodo.dof[2]] = nodo.restraints[2]
...
def solve(modelo: "SystemMilcaModel") -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    K_red, F_red, dofs_libres = procesar_condiciones(modelo)
